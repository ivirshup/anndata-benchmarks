...
class SubsetMemorySuite:
    ...
    def track_repeated_subset_memratio(
        self, n_obs, n_var, attr_set, subset_dim, index_kind
    ):
        nviews = 5
        base_size = get_anndata_memsize(self.adata)
        mem_recording = memory_usage(
...
class SubsetTimingSuite:
    ...
    # def setup(self, n_obs, n_var, attr_set, subset_dim, index_kind):
    def setup(self, n_obs, n_var, subset_dim, index_kind):
        adata = helpers.gen_adata((n_obs, n_var))
        # adata = gen_adata(n_obs, n_var, attr_set)
        self.adata = adata
        self.subset = gen_indexer(adata, subset_dim, index_kind, ratio=0.5)

    def time_subset(self, n_obs, n_var, subset_dim, index_kind):
        v = self.adata[self.subset]

    def time_subset_X(self, n_obs, n_var, subset_dim, index_kind):
        Xv = self.adata[self.subset].X

    def time_copy_subset(self, n_obs, n_var, subset_dim, index_kind):
